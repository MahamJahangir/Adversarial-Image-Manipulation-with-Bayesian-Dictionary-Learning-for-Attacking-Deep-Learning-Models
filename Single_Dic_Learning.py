...
def sample_Dk(k,Xd, D, Sd, g_eps_d, Z):
    ...
    return Xd, D
def sample_ZSk(k,Xd, D, Sd, Z, Pid, g_sd, g_eps_d):
    Xdd = Xd + D.dot(Z.T*Sd.T)
    DTD = np.sum(D** 2)
...
def dictionary_learning(training_samples,dicSize,sparsity,iterations):
    #starttime = time.time()
    a0=1.0
    b0=1.0
    c0=60
    ...
    while iter<iterations:
        ...
        for k in range(D.shape[1]):
            ...
            Xd,Sd[:,k:k+1],Z[:,k:k+1] = sample_ZSk(k,Xd, D[:,k:k+1], Sd[:,k:k+1], Z[:,k:k+1], Pid[0,k], g_sd, g_eps_d)
